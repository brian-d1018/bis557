...
data(iris)
print(b_gd <- bis557::ols_gd_hw2b(form = Sepal.Length ~ ., d = iris[,-5], 
                                  b_0 = rep(1e-9,4), learn_rate = 2, 
                                  max_iter = 2e5))
print(lm(Sepal.Length ~ ., iris[,-5])$coefficients)
```

Here, we see that the penalty based on the out-of-sample accuracy is estimated 
to be around $27.30$, which is not bad considering that the `iris` dataset has 
150 observations. 
...
data(iris)
print(lm(Sepal.Length ~ ., iris)$coefficients)
b_ridge <- bis557::ridge_hw2c(form = Sepal.Length ~ ., d = iris, 
                              lambda_val = 1e3)
print(b_ridge$coefficients)

# Show that ridge regression works for colinear regression variables
data(lm_patho)
print(bis557::ridge_hw2c(form = y ~ ., d = lm_patho, lambda_val = 1e8))
```
